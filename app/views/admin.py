...
try:
    ...
    JY_AVAILABLE = False
    print("Warning: JianYingApi import failed; draft parsing will fall back to JSON only.")
...
@admin_bp.route("/")
def admin_home():
    return render_template("user/admin.html", site_settings=get_site_settings())
...
def admin_token_required():
    ...
    if err:
        return None, jsonify({"ok": False, "error": "Admin token is invalid or expired"}), 401
    if user.role != "admin":
        return None, jsonify({"ok": False, "error": "Admin privileges required"}), 403
    ...
    data = [
        {
            ...
            "vip_expire_at": quotas.get(u.id).vip_expire_at.isoformat()
            if quotas.get(u.id) and quotas.get(u.id).vip_expire_at
            else None,
        }
        for u in users
    ]
...
def api_delete_user(id):
    if id == session["user_id"]:
        return jsonify({"success": False, "error": "Cannot delete the current user"})
...
def admin_update_quota():
    ...
    if vip_expire_at:
        try:
            ...
            return jsonify({"ok": False, "error": "vip_expire_at format is invalid"}), 400
    elif vip_expire_at == "":
        vip_dt = None
...
def admin_update_quota_batch():
    ...
    for item in items:
        try:
            ...
            if not user_id:
                results.append({"ok": False, "error": "Missing user_id"})
                continue
            ...
            if vip_expire_at:
                try:
                    ...
                    results.append({"ok": False, "user_id": user_id, "error": "vip_expire_at format is invalid"})
                    continue
            elif vip_expire_at == "":
                vip_dt = None
...
@admin_required
def api_logs():
    logs = read_generate_logs(limit=1000)
